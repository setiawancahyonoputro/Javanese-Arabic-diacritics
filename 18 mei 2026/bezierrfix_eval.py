...
TSP_SMALL_DIACRITIC_LABEL_POINT_MAX = 34
TSP_SMALL_DIACRITIC_LABEL_BBOX_MAX = 18
TSP_SMALL_DIACRITIC_LABEL_BODY_RATIO_MAX = 0.18
...
def hybrid_tsp_split_until_stable(G_total, nodes_set, baseline_y=None, r_stroke=1.5,
    ...
    Return:
      parts    : list set(node)
      cut_rows : list metadata cut TSP
    """
    ...
    while len(cut_rows) < max_splits:
        ...
        for i, part in enumerate(parts):
            if len(part) < 2 * int(max(TSP_MIN_LETTER_SIZE, HYBRID_TSP_MIN_LETTER_SIZE)):
                continue
            Gp = G_total.subgraph(part).copy()
...
def hybrid_tsp_refine_labels_with_arabic_cut(skeleton_img, labels_img,
    ...
    for old_lab in old_label_ids:
        ...
        for cc_idx, cc_nodes in enumerate(body_ccs, start=1):
            if len(cc_nodes) < 2 * int(max(TSP_MIN_LETTER_SIZE, HYBRID_TSP_MIN_LETTER_SIZE)):
                raw_parts.append(set(cc_nodes))
                continue
        ...
        # Jika tidak ada bridge TSP yang benar-benar memotong label, semua body
        # fragment tetap satu huruf. Ini menjaga struktur lama agar tidak rusak.
        if split_applied:
            final_parts = raw_parts
        else:
    ...
    if np.any(residual > 0):
        ...
        for rid in range(1, n_res):
            ...
            best_idx, best_dist = hybrid_tsp_assign_residual_component_to_nearest_part(
                comp_nodes,
                part_items,
                old_labels=[int(v) for v in old_vals.tolist() if int(v) > 0],
            )
            if best_idx is None:
                continue
    ...
    if debug:
        ...
        split_count = sum(1 for r in rows if r.get("assignment_type") == "hybrid_arabic_cut_tsp_bridge_split")
        print(
...
def tsp_write_debug_outputs(imagename, output_dir, csv_dir, source_skeleton, labels_img,
    ...
    try:
        ...
        ax.imshow(sk, cmap='gray', alpha=0.25)
        ...
        ax.set_title("TSP sebelum Bezier: body dan diakritik sebagai sub-path terpisah")
        ax.axis('off')
        plt.tight_layout()
...
SELECTIVE_GAP_MAX_DISTANCE = 6.0       # jarak endpoint maksimum yang dianggap gap kecil (px)
...
SELECTIVE_GAP_DILATE_ITER = 1          # sesuai alur: dilation 1 hanya pada area gap
...
SELECTIVE_GAP_MERGE_LABELS_TOUCHED_BY_BRIDGE = False
...
if USE_GATE_OTSU_BINARY:
    ...
    if USE_CONNECTED_COMPONENT_MIN_SIZE_NOISE_FILTER:
        gray = connected_component_min_size_noise_filter(
            ...
            debug=False,
            title="CC filter awal tanpa erosion",
        )
