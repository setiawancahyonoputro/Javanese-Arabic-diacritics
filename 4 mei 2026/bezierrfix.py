...
if combined_skeleton is not None and np.any(combined_skeleton > 0):
    ...
    # ============================================================
    # ============================================================
    # BEZIER CURVE PER HURUF DARI SKELETON
    # Output dibuat seperti contoh: satu plot kurva untuk tiap huruf/komponen.
    # Kurva tidak lagi satu garis global; setiap connected component skeleton
    # direkonstruksi menjadi Bezier piecewise.
    # ============================================================
    bezier_curves = {}
    SHOW_BEZIER_OVERLAY = False   # True jika masih ingin output overlay gabungan seperti gambar 1
    SHOW_SKELETON_POINTS = True   # titik skeleton dipakai sebagai referensi, ditampilkan samar

    if len(sig_curves) > 0:
        print("\n" + "="*60)
        print("BEZIER CURVE PER HURUF DARI TITIK SKELETON")
        print("="*60)
        ...
        # 1) Bentuk kurva Bezier per komponen dari ordered skeleton Zhang-Suen.
        for disp_idx, (sig_obj, _sig_curve, cv_id, x_left) in sorted(sig_curves.items()):
            ordered_pts = sig_obj.ordered_points
            if ordered_pts is None or len(ordered_pts) < 2:
                print(f"  Huruf {disp_idx}: skip, titik skeleton kurang.")
                continue

            mask_comp = ((labels_sig == cv_id).astype(np.uint8) * 255)
            try:
                curve_pts, control_segments, closed_flag = make_bezier_curve_per_letter_from_ordered_skeleton(
                    ordered_pts,
                    mask_comp=mask_comp,
                ...
                if len(curve_pts) < 2:
                    print(f"  Huruf {disp_idx}: skip, kurva kosong.")
                    continue

                fit_eval = evaluate_curve_fitting_to_skeleton(ordered_pts, curve_pts)
                ...
                bezier_curves[disp_idx] = {
                    ...
                    "skeleton": np.asarray(ordered_pts, dtype=float),
                ...
                print(f"  Huruf {disp_idx}: OK | skeleton={len(ordered_pts)} titik | "
                      f"segmen={len(control_segments)} | curve={len(curve_pts)} titik | "
                      f"closed={closed_flag} | RMSE={fit_eval['rmse_skeleton_to_curve']:.4f} px | "
                ...
                if PRINT_BEZIER_PER_HURUF_CALCULATION_OUTPUT:
                    # Output numerik per huruf dengan format seperti bezier.py.
                    # Pada Bezier per-huruf, curve_pts sudah berada di koordinat gambar,
                    # sehingga xx_sc diisi dengan titik curve_pts juga.
                ...
                if SAVE_BEZIER_OUTPUT_CSV and BEZIER_CSV_DIR is not None:
                    ...
                    bezier_csv_summary_rows.append(
                        build_bezier_summary_row(
                            ...
                            curve_pts,
                            huruf=disp_idx,
                            skeleton_points=int(len(ordered_pts)),
                            segment_count=int(len(control_segments)),
                            curve_points=int(len(curve_pts)),
                            closed=bool(closed_flag),
                            cv_label=int(cv_id),
        ...
        # 2) Output utama: satu subplot per huruf, seperti contoh gambar 2.
        ...
        if len(valid_keys) > 0:
            ...
            for plot_idx, disp_idx in enumerate(valid_keys):
                ...
                ax.plot(curve_pts[:, 0], curve_pts[:, 1], '-', linewidth=1.1)
                if SHOW_SKELETON_POINTS:
                    ax.scatter(skel_pts[:, 0], skel_pts[:, 1], s=3, alpha=0.22)

                ax.set_title(f"Huruf {disp_idx}", fontsize=8)
            ...
            fig_grid.suptitle(
                "Kurva Bezier per Huruf dari Skeleton Zhang-Suen (Satu Plot per Huruf)",
                fontsize=12
            )
            fig_grid.text(
                0.5, 0.015,
                "Titik kontrol diambil dari skeleton; kurva piecewise mengikuti lintasan skeleton.",
            ...
            # 3) Simpan juga plot individual per huruf, tanpa background gambar asli.
            for disp_idx in valid_keys:
                ...
                ax_one.plot(curve_pts[:, 0], curve_pts[:, 1], '-', linewidth=1.4)
                if SHOW_SKELETON_POINTS:
                    ax_one.scatter(skel_pts[:, 0], skel_pts[:, 1], s=4, alpha=0.22)
        ...
        # 4) Overlay gabungan hanya opsional. Default False supaya tidak tampil seperti gambar 1.
        if SHOW_BEZIER_OVERLAY and len(bezier_curves) > 0:
            ...
            skel_y_bz, skel_x_bz = np.where(combined_skeleton > 0)
            ...
            for i, disp_idx in enumerate(sorted(bezier_curves.keys())):
                curve_pts = bezier_curves[disp_idx]["curve"]
                color = cmap_bz[i % len(cmap_bz)]
                ax_bz.plot(curve_pts[:, 0], curve_pts[:, 1], '-', color=color,
                           linewidth=2.0, label=f'Huruf-{disp_idx}')

            ax_bz.set_title("Overlay Kurva Bezier per Huruf dari Skeleton Zhang-Suen")
            ax_bz.axis('off')
