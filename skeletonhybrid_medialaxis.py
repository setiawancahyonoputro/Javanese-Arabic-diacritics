...
# ====== SKELETON: ZHANG vs MAT vs DT-RIDGE vs HYBRID ======
SKELETON_MAIN = "hyb"   # pilih: "zhang" / "mat" / "dt" / "hyb"

skeleton_components_zhang = []
...
for n in range(len(components)):
    ...
    # simpan hole per komponen (wawu)
    holes = get_holes_mask(binary_mat)

    # close utk huruf besar, open utk dot
    ...
    # kunci: lubang tetap jadi background
    binary_mat[holes == 255] = 0
    fg = (binary_mat > 0)

    # A) Zhang (baseline lama)
    skel_zh = skeletonize(fg, method='zhang').astype(np.uint8)
    skeleton_components_zhang.append(skel_zh)

    # B) MAT
    skel_mat_bool, _dist_mat = medial_axis(fg, return_distance=True)
    skel_mat = skel_mat_bool.astype(np.uint8)
    skeleton_components_mat.append(skel_mat)

    # C) Pure DT ridge
    skel_dt, _dist_dt = dt_ridge_skeleton(fg, r_min=1.0, neigh=3)
...
def combine_list(skel_list):
    if len(skel_list) == 0:
        return None
    out = np.zeros_like(skel_list[0], dtype=np.uint8)
    for s in skel_list:
        out |= (s > 0).astype(np.uint8)
    return out

combined_zhang = combine_list(skeleton_components_zhang)
combined_mat   = combine_list(skeleton_components_mat)
combined_dt    = combine_list(skeleton_components_dt)
combined_hyb   = combine_list(skeleton_components_hyb)

# pilih skeleton utama untuk dipakai di tahap DT evaluasi di bawah
if SKELETON_MAIN == "zhang":
    combined_skeleton = combined_zhang
elif SKELETON_MAIN == "mat":
    combined_skeleton = combined_mat
elif SKELETON_MAIN == "dt":
    combined_skeleton = combined_dt
else:
    combined_skeleton = combined_hyb

# visual cepat
if combined_skeleton is not None and np.any(combined_skeleton > 0):
    plt.figure(figsize=(14, 4))
    plt.subplot(1,4,1); plt.imshow(combined_zhang, cmap='gray'); plt.title("Zhang"); plt.axis('off')
    plt.subplot(1,4,2); plt.imshow(combined_mat,   cmap='gray'); plt.title("MAT");   plt.axis('off')
    plt.subplot(1,4,3); plt.imshow(combined_dt,    cmap='gray'); plt.title("DT ridge"); plt.axis('off')
    plt.subplot(1,4,4); plt.imshow(combined_hyb,   cmap='gray'); plt.title("Hybrid"); plt.axis('off')
    plt.tight_layout(); plt.show()

    plt.figure(figsize=(6,6))
    plt.imshow(combined_skeleton, cmap='gray')
    plt.title(f"Hybrid skeleton Medialaxis+Distance Transform")
    plt.axis('off')
    plt.show()
else:
