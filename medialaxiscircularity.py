...
def contour_dt_to_edge(bin255):
    """
    bin255: binary foreground 0/255 (huruf=255, bg=0)
    Return:
      - contour_img: tepi 1-pixel (255 di tepi)
      - dt_edge: float32, jarak tiap piksel ke tepi (kontur) terdekat
    """
    bin255 = ((bin255 > 0).astype(np.uint8) * 255)

    # Kontur termasuk hole: RETR_CCOMP / RETR_TREE
    contours, hier = cv.findContours(bin255, cv.RETR_CCOMP, cv.CHAIN_APPROX_NONE)

    contour_img = np.zeros_like(bin255, dtype=np.uint8)
    if len(contours) > 0:
        cv.drawContours(contour_img, contours, -1, 255, 1)  # 1-pixel boundary

    # distanceTransform menghitung jarak ke piksel 0 terdekat,
    # jadi jadikan kontur sebagai 0, lainnya 255.
    src = np.full_like(contour_img, 255, dtype=np.uint8)
    src[contour_img > 0] = 0

    dt_edge = cv.distanceTransform(src, cv.DIST_L2, 5)  # float32
    return contour_img, dt_edge


def dt_map_at_skeleton(skel_u8, dt_edge):
    """
    skel_u8: 0/1 atau 0/255
    dt_edge: float32 hasil contour_dt_to_edge
    Return:
      - dt_skel_map: float32, hanya bernilai di titik skeleton
      - dist_vals: array nilai DT pada titik skeleton
    """
    sk = (skel_u8 > 0)
...
def _bbox_extent_roundness(mask_u8):
    """
    Ekstrak fitur bentuk tambahan untuk nolak 'band/garis':
    - bbox_ratio: min(w,h)/max(w,h)  (mendekati 1 = lebih bulat)
    - extent: area_contour / (w*h)   (mendekati 1 = lebih padat mengisi bbox)
    """
    mu8 = ((mask_u8 > 0).astype(np.uint8) * 255)
    cnts, _ = cv.findContours(mu8, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE)
    if not cnts:
...
def carve_pseudo_holes_by_dt_peaks(
    ...
    """
    Deteksi gumpalan bulat lokal dengan filter berbasis distribusi r0.
    Blob sejati = peak dengan r0 JAUH di atas median r0 stroke (bukan sekadar stroke tebal).
    """
    ...
    # === KUNCI: hitung distribusi r0 semua peaks ===
    # median r0 = tebal "normal" stroke di komponen ini
    # blob sejati harus punya r0 > median * r0_blob_factor
    all_r0 = dist[ys, xs]
    r0_median = float(np.median(all_r0))
    r0_blob_thresh = r0_median * 1.15  # blob harus 15% lebih tebal dari stroke normal
    if debug:
    ...
    for k in order:
        ...
        # Filter 1: r0 harus di atas r_peak_min
        if r0 < r_peak_min:
            continue

        # Filter 2 (UTAMA): r0 harus jauh di atas median stroke
        if r0 < r0_blob_thresh:
            break   # peak sudah diurutkan desc, yang berikutnya pasti lebih kecil

        # Filter 3: Prominence check - blob sejati punya r0 yang "menonjol" dari sekitarnya
        # Persimpangan stroke: r0 tinggi tapi tetangganya juga hampir setinggi itu
        # Blob sejati: r0 tinggi, tetangganya jauh lebih rendah (r0 drop tajam ke tepi blob)
        # Ukur: ambil DT di ring r0*0.6 s.d. r0*0.9 sekitar peak -> rata2 harus < r0 * 0.75
        ...
        if _ring_mask.sum() > 0:
            ...
            # Jika tetangga juga hampir setinggi r0 = persimpangan stroke, bukan blob
            if _r0_ring_mean > r0 * 0.80:
                continue

        # Filter 4: hitung cabang skeleton yang keluar dari area peak
        # Junction T/Y = 3+ cabang terpisah. Blob loop = 0-2 cabang
        _skel_tmp = skeletonize(fg_new).astype(np.uint8)
        ...
        if extent < extent_min:
            continue
        # Filter elongasi keras: core di tikungan stroke sering lonjong
        # bbox_ratio = min(w,h)/max(w,h) harus > 0.45 (tidak terlalu memanjang)
        if bbox_ratio < 0.45:
            continue
        # Rasio sisi bbox tidak boleh > 2.2x (anti-stroke-melengkung)
...
def create_circular_skeleton_from_dt(fg_bool, min_radius=2.0, dt_percentile=60, force_circular=False):
    """
  ...
  # GUARD: kalau memanjang → skip circular, pakai MAT+DT biasa
    # Guard DILEWATI jika force_circular=True (komponen sudah diverifikasi circular_blob)
...
def evaluasi_skeleton_lengkap(skeleton, edge, dt_edge, component_types):
    ...
    # connected components skeleton (punyamu boleh tetap)
...
def detect_blob_type(binary_mat, cy, baseline_y, median_body_h):
    ...
    # Hitung bbox ratio untuk tambahan konfirmasi kебulatan
    bbox_ratio, extent, bw_bbox, bh_bbox = _bbox_extent_roundness(binary_mat)
    
    # Kriteria utama: circularity + axis ratio + bbox cukup bulat
    # Range area diperlebar: 150 s/d 5000 untuk tangkap blob kecil & besar
    # dy_thresh toleransi lebih besar (1.8x) untuk blob yang agak dekat body
    if (150 <= area_px <= 5000 and
        circ > 0.38 and                  # lebih rendah dari sebelumnya
        axis > 0.38 and                  # lebih rendah dari sebelumnya
        bbox_ratio > 0.35 and            # tidak terlalu memanjang
        dy <= dy_thresh * 1.8):          # toleransi lebih besar
...
skeleton_components_ski = []
skeleton_components_mat = []
skeleton_components_dt  = []
skeleton_components_hybrid = []
...
for n in range(len(components)):
    ...
    # Erosi 1x untuk menipiskan stroke sebelum skeleton
    # Stroke tebal menghasilkan skeleton gumpal - erosi tipis dulu
    _k_erosi = cv.getStructuringElement(cv.MORPH_ELLIPSE, (2, 2))
    _fg_eroded = cv.erode(binary_mat, _k_erosi, iterations=1)
    _fg_eroded[holes == 255] = 0  # jaga holes tetap ada
    fg = (_fg_eroded > 0)
    ...
    # ===== Baseline skeleton methods (untuk pembanding) =====
    skel_ski = skeletonize(fg_used).astype(np.uint8)
    
    skel_mat_bool, _ = medial_axis(fg_used, return_distance=True)
    skel_mat = thin(skel_mat_bool).astype(np.uint8)   # tipiskan agar 1px
    
    skel_dt, _ = dt_ridge_skeleton(fg_used, r_min=1.0, neigh=3)
    skel_dt = (skel_dt > 0).astype(np.uint8)          # pastikan 0/1
    ...
    if blob_type == 'diacritic':
        ...
        skel_mat_bool, _ = medial_axis(fg_used, return_distance=True)
        skel_hybrid = thin(skel_mat_bool).astype(np.uint8)
        skel_hybrid = skeletonize(skel_hybrid > 0).astype(np.uint8)
        ...
        if np.count_nonzero(holes) == 0:
            fg_used, forced_loop = carve_pseudo_holes_by_dt_peaks(
                fg,
                peak_neigh=5,        # lebih kecil agar sensitif pada blob kecil
                r_peak_min=2.0,      # diturunkan agar blob kecil bisa di-carve
                alpha=0.65,          # diturunkan agar core tidak terlalu kecil
                circ_min=0.35,       # lebih longgar
                axis_min=0.35,       # lebih longgar
                max_carves=2,
                min_core_area=12,    # diturunkan untuk blob kecil
                extent_min=0.20,     # lebih longgar
                bbox_min=0.30,       # lebih longgar
                core_ratio_max=0.80, # lebih longgar
                max_len_vs_r0=8.0    # lebih longgar
            )
            if forced_loop:
                print(f"  → Pseudo-hole berhasil di-carve!")
        
        # Buat skeleton melingkar - force_circular=True karena sudah diverifikasi sebagai circular_blob
        # Tidak perlu cek ulang circularity, guard di dalam fungsi dilewati
        skel_circular = create_circular_skeleton_from_dt(fg_used, min_radius=2.0, dt_percentile=60, force_circular=True)
    ...
    # HURUF NORMAL: hybrid medial + DT + deteksi circular blob lokal (DT-core)
        ...
        # Hitung area total & DT max komponen untuk adaptasi parameter
        # Filter blob via distribusi r0: blob sejati = r0 jauh di atas median stroke
        _fg_total_area = float(fg.sum())
        fg_used, forced_loop, cores = carve_pseudo_holes_by_dt_peaks(
            fg,
            peak_neigh=7,
            r_peak_min=2.0,     # scan semua peak >= 2px
            alpha=0.72,
            circ_min=0.38,      # moderat - sudah diproteksi r0_blob_thresh
            axis_min=0.38,
            extent_min=0.25,
            bbox_min=0.30,
            core_ratio_max=0.80,
        ...
        # Filter: buang core > 40% area komponen (bukan gumpalan lokal)
        if len(cores) > 0:
            _filtered = []
            for _cm in cores:
        ...
        # Skeleton dari fg ASLI - sudah 1px via thin()
        skel_mat_orig, _ = medial_axis(fg, return_distance=True)
        skel_thin_orig = skeletonize(skel_mat_orig).astype(np.uint8)

        if len(cores) > 0:
            # Area cores yang diperluas
            ...
            # Skeleton dari fg_used (berlubang) - juga di-thin dulu
            skel_mat_carved, _ = medial_axis(fg_used, return_distance=True)
            skel_thin_carved = skeletonize(skel_mat_carved).astype(np.uint8)
            # Ambil hanya bagian carved yang di dalam area core
            skel_core_only = (skel_thin_carved > 0) & cores_mask
            # Gabung: orig + core loop, keduanya sudah 1px
            skel_hybrid = thin(
                (skel_thin_orig > 0) | skel_core_only
            ).astype(np.uint8)
        else:
            skel_hybrid = skel_thin_orig
        # Pastikan skeleton benar-benar 1px dengan skeletonize ulang
        skel_hybrid = skeletonize(skel_hybrid > 0).astype(np.uint8)
        print(f"  → Skeleton: Hybrid MAT+DT (normal)")

    skeleton_components_ski.append(skel_ski)
    skeleton_components_mat.append(skel_mat)
...
# ===== Gabungkan semua skeleton komponen (SEMUA METODE) =====
H, W = height, width

combined_ski = combine_component_skeletons(skeleton_components_ski, (H, W)) if len(skeleton_components_ski) else np.zeros((H, W), np.uint8)
combined_mat = combine_component_skeletons(skeleton_components_mat, (H, W)) if len(skeleton_components_mat) else np.zeros((H, W), np.uint8)
combined_dt  = combine_component_skeletons(skeleton_components_dt,  (H, W)) if len(skeleton_components_dt)  else np.zeros((H, W), np.uint8)
combined_hybrid = combine_component_skeletons(skeleton_components_hybrid, (H, W)) if len(skeleton_components_hybrid) else np.zeros((H, W), np.uint8)

# biar kode bawah kamu yang masih pakai combined_skeleton tetap jalan
combined_skeleton = combined_hybrid

# PASTIKAN skeleton benar-benar 1px - skeletonize ulang hasil akhir
# Ini mengatasi kasus di mana thin() tidak sempurna pada area tebal
from skimage.morphology import skeletonize
_skel_bool = (combined_skeleton > 0)
_skel_bool = skeletonize(_skel_bool)  # skeletonize lebih kuat dari thin()
combined_skeleton = (_skel_bool.astype(np.uint8) * 255)
combined_hybrid = combined_skeleton
...
# TEPI (BOUNDARY) dari binary + DT (jarak ke tepi terdekat)
# =============================================================
print("Menghitung kontur (boundary) & DT (jarak ke tepi terdekat)...")

# gray diasumsikan binary 0/255
bin255 = ((gray > 0).astype(np.uint8) * 255)

# Ambil kontur dari foreground binary (outer + hole)
contours, hierarchy = cv.findContours(bin255, cv.RETR_CCOMP, cv.CHAIN_APPROX_NONE)

# Gambar kontur 1-pixel
contour_img = np.zeros_like(bin255, dtype=np.uint8)
if len(contours) > 0:
    cv.drawContours(contour_img, contours, -1, 255, 1)

# distanceTransform: jarak ke piksel 0 terdekat
# jadi buat image: kontur=0, selainnya=255
src = np.full_like(contour_img, 255, dtype=np.uint8)
src[contour_img > 0] = 0

dt = cv.distanceTransform(src, cv.DIST_L2, 5)  # float32
dt_edge = dt

# edges untuk evaluasi overlap (pakai boundary)
edges = (contour_img > 0).astype(np.uint8)


# ===== Plot DT pada skeleton untuk semua metode =====
methods = {
    "SKIMAGE skeletonize": combined_ski,
    "Medial Axis": combined_mat,
    "DT Ridge": combined_dt,
    "Hybrid (MAT+DT+blob)": combined_hybrid,
}
...
for name, sk in methods.items():
    ...
    contour_rgba[contour_img > 0] = [1.0, 1.0, 1.0, 1.0]  # putih, opaque penuh
    plt.imshow(contour_rgba)
    plt.imshow(contour_img, cmap="gray", alpha=0.35)
    plt.title(f"DT on Skeleton (skeleton -> nearest edge) | {name}")
    plt.axis("off")
    plt.show()
